...
> 
> 2021-12-08：声音小的原因是阻抗不匹配，PCI-E 声卡需要一个增益输出，而我们日常使用的普通麦克风没有增益，输出功率太小，很难被声卡捕捉。如何改善这种情况 Charlie 会找机会测试，至于测试的结果不知道要鸽到什么时候，所以还是尽量考虑 USB 声卡吧。
> 
> 2022-05-30：原因找到了，PCI-E 声卡需要在 Config ASIO 环节选定特定输入通道，否则就会出现录不上音的情况。（这么简单的问题我之前咋就没发现）

如上图所示的声卡型号是 `创新科技（Creative）Sound BlasterX AE-5` 。在京东上的价格为 1049 。这个价格并不比既可录音也可播放声音的 USB声卡 便宜。
